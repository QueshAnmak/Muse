...
async function searchQuery(ymusic, query) {
    ...
    await ymusic.waitForSelector('#search-page > ytmusic-tabbed-search-results-renderer > div.content.style-scope.ytmusic-tabbed-search-results-renderer');
...
async function decideMusic(ymusic, musicType='') {
    ...
    await ymusic.evaluate(async (musicType) => {
        ...
        const topResult = {};
        topResult.musicType = await document.querySelector(' ytmusic-shelf-renderer > #contents > ytmusic-responsive-list-item-renderer > .flex-columns > .secondary-flex-columns > yt-formatted-string > span ').innerText;
        // this should instead point to whatever element plays the top result
        // this wont work for artist
        topResult.playButton = await document.querySelector(' ytmusic-shelf-renderer > #contents > ytmusic-responsive-list-item-renderer ').querySelector(' .ytmusic-play-button-renderer ');
        ...
        if (topResult.musicType.toLowerCase() === musicType) {
            ...
            const getTitlesList = async () => {
                const titles = await document.querySelector(' .content.style-scope.ytmusic-tabbed-search-results-renderer ').querySelectorAll(' h2 ');
                const titleSelectors = {}
                for (idx in titles){
                    var titleName = titles[idx].innerText;
                    if (typeof(titleName) == 'string') {
                        var firstItemPlayButton = await titles[idx].parentElement.querySelector('.ytmusic-play-button-renderer');
                        titleSelectors[titleName.toLowerCase()] = firstItemPlayButton;
                    }
                }
...
async function getCurrentSong(ymusic) {
    ...
    await ymusic.waitForSelector('yt-formatted-string[class="byline style-scope ytmusic-player-bar complex-string"]');

    const songName = await ymusic.evaluate(async () => {
        return await document.querySelector('yt-formatted-string[class="title style-scope ytmusic-player-bar"]').title;
    });
    const { songArtist, songAlbum } = await ymusic.evaluate(async () => {
        const otherData = await document.querySelector('yt-formatted-string[class="byline style-scope ytmusic-player-bar complex-string"]');
        const otherSongData = {}
...
async function toggleMusic(ymusic) {
    ...
    await ymusic.click("#play-pause-button")
    const playerState = await ymusic.getAttribute('#play-pause-button', 'title');
...
async function main() {
    ...
    await playMusic(ymusic, "iauydvbklaiudblaidufsblaiuwdfbaiydbf;uiawsgbdg;klasgdlfvakiusdhfwoey9r5h243f")
